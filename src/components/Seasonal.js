...
export default function Seasonal({addToCart}){

  const [veg, setveg] = useState([]);
  const loadData = async () => {
    let response = await fetch("http://localhost:5000/api/product/Seasonal", {
      method: "GET",
      headers: {
  ...
  return (
    <>
      <h2 className='item-heading'>Seasonal Fruits</h2>
      <div className="body">
